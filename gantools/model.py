...
class BiWGAN(BaseGAN):
    ...
    def wgan_regularization(self, gamma, list_fake_x, list_real_x, list_fake_z, list_real_z):
        if not gamma:
            ...
            # calculate `x_hat`
            assert(len(list_fake_x) == len(list_real_x))
            bs = tf.shape(list_fake_x[0])[0]
            eps = tf.random_uniform(shape=[bs], minval=0, maxval=1)
            x_hat = []
            for fake, real in zip(list_fake_x, list_real_x):
                singledim = [1]* (len(fake.shape.as_list())-1)
                eps = tf.reshape(eps, shape=[bs,*singledim])
                x_hat.append(eps * real + (1.0 - eps) * fake)

            bs = tf.shape(list_fake_z[0])[0]
            eps = tf.random_uniform(shape=[bs], minval=0, maxval=1)
            z_hat = []
            for fake, real in zip(list_fake_z, list_real_z):
                single_dim = [1] * (len(fake.shape.as_list()) - 1)
                eps_reshaped = tf.reshape(eps, shape=[bs, *single_dim])
                z_hat.append(eps_reshaped * real + (1.0 - eps_reshaped) * fake)

            D_x_hat = self.discriminator(*x_hat, *z_hat, reuse=True)
...
def discriminator(x, z, params, reuse=True, scope="discriminator"):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if params['non_lin']:
            non_lin_f = getattr(tf, params['non_lin'])
            x = non_lin_f(x)
            rprint('    Non linearity: {}'.format(params['non_lin']), reuse)
